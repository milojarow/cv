...
export default function CardCV() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsLanguageToggleVisible(entry.isIntersecting);
      },
      { threshold: 0.1 }
    );
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white shadow-xl rounded-lg overflow-hidden cv-card">
      ...
      <div className="fixed bottom-6 right-6 flex flex-col space-y-4 z-50 no-print">
        {/* Language Toggle (only when header toggle is not visible) */}
